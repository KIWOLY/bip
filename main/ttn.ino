...
{
    DEBUG_PORT.println(count);
    if (count == 0)
    {
        DEBUG_PORT.println("in if");
    ...
    DEBUG_PORT.println("einde initcount");
}

bool ttn_setup()
{
    Serial.println("Init count");
    initCount();
    Serial.println("Init count einde");
#if defined(USE_OTAA)
    initDevEUI();
#endif
    ...
    DEBUG_PORT.println("spi initialized");
    // LMIC init
...
void ttn_join()
{
    ...
    Preferences p;
    Serial.println("lookup lora in flash");
...
static void ttn_set_cnt()
{
    ...
    { // write if we roll over (50 days) or 5 mins
        ...
        {
            DEBUG_PORT.println("prefs init FAILED");
        }
    }
}
